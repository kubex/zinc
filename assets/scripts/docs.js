...
(() =>
{
  ...
  {
    const menu = document.querySelector('#theme-selector zn-menu');
    if(!menu)
    {
      return;
    }
    [...menu.querySelectorAll('zn-menu-item')].map(item => (item.checked = item.getAttribute('value') === theme));
  }

  let theme = getTheme();

  // Selection is not preserved when changing page, so update when opening dropdown
  document.addEventListener('zn-show', event =>
  {
...
(() =>
{
  function updateVersion()
  {
    ...
    if(location.hostname === 'next.zinc.style')
    {
      el.textContent = 'Next';
    }
