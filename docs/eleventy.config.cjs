...
const zincFlavoredMarkdown = require('./_utilities/markdown.cjs');
...
module.exports = function (eleventyConfig)
{
  ...
  //
  eleventyConfig.setLibrary('md', zincFlavoredMarkdown);
  ...
  {
    let name = capitalCase(className.replace(/^Zn/, ''));
    if(name === 'Qr Code')
    {
      name = 'QR Code';
    } // manual override
  ...
  eleventyConfig.addFilter('removeZNPrefix', tagName =>
  {
    return tagName.replace(/^zn-/, '');
  });
  ...
  eleventyConfig.addTransform('html-transform', function (rawContent)
  {
    let content = replacer(rawContent, [
      ...
      {pattern: '%OUTDIR%', replacement: outDir}
    ]);
  ...
  //
  // Send a signal to stdout that lets the build know we've reached this point
  //
