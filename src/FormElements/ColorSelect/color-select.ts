...
export class ColorSelect extends EmptyZincFormElement
{
  ...
  private handleDocumentKeyDown = (event: KeyboardEvent) =>
  {
    ...
    if(event.key.length === 1 || event.key == 'Backspace')
    {
      ...
      {
        const label = options.textContent.toLowerCase();
  ...
  {
    return deepQuerySelectorAll('.select__option', this, null) as HTMLElement[];
  }
  ...
  handleValueChange()
  {
    ...
    this.setSelectedOptions(allOptions.filter(el => value.includes(el.getAttribute('data-key'))));
  }
