...
export class SubmenuController implements ReactiveController {
  private host: ReactiveControllerHost & ZnMenuItem;
  private popupRef: Ref<ZnPopup> = createRef();
  ...
  constructor(
    host: ReactiveControllerHost & ZnMenuItem,
  ...
  private handleClick = (event: MouseEvent) => {
    ...
    if (event.target === this.host) {
      ...
      (event.target.tagName === 'zn-menu-item' || event.target.role?.startsWith('menuitem'))
    ) {
      this.disableSubmenu();
    }
  };
  ...
  renderSubmenu() {
    ...
    // Always render the slot, but conditionally render the outer <zn-popup>
    if (!this.isConnected) {
      return html`
