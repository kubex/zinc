...
export default class ZnQueryBuilder extends ZincElement implements ZincFormControl {
  ...
  private _addRule(event: Event | null, value: string, pos: any = null) {
    ...
    const filter: any = this.filters.find(item => item.id === id);
    if (!filter) return;
    ...
    filter.operators.forEach((item: any) => {
      const option = document.createElement('zn-option') as ZnOption;
      option.value = item;
      switch (item) {
        case 'eq':
    ...
    if (filter.type === 'date') {
      ...
      comparator.addEventListener('change', (e: Event) => {
        ...
        const input = row.querySelector('.query-builder__value') as HTMLInputElement;
        const parent = input.parentElement as HTMLDivElement;
        const filter = this._selectedRules.get(uniqueId) as CreatedRule;
      ...
      comparator.addEventListener('zn-change', (e: Event) => {
        ...
        if (selectedComparator !== previousOperator && selectedComparator === 'in') {
          ...
          const options: any = this.filters.find(item => item.id === filter?.id)?.options;
  ...
  private _updateValue(id: string, event: Event | { target: ZnSelect | HTMLInputElement | HTMLDivElement }) {
    const filter = this._selectedRules.get(id);
    if (!filter) return;
